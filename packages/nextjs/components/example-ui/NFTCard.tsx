...
export const NFTCard = ({ tokenId }: NFTCardProps) => {
  ...
  useEffect(() => {
    if (marketplaceAddress) {
      ...
      console.log(`[NFT #${tokenId}] Approval Status Check:`, {
        approvedAddress,
        marketplaceAddress,
        isApprovedForAll,
        finalIsApproved: approved,
        isApproving,
      });
      setIsApproved(approved);
    }
  }, [approvedAddress, isApprovedForAll, marketplaceAddress, tokenId, isApproving]);
  ...
  const { writeAsync: approveMarketplace } = useScaffoldContractWrite({
    ...
    onBlockConfirmation: async (txnReceipt: any) => {
      console.log(`[NFT #${tokenId}] ✅ onBlockConfirmation fired!`, {
      ...
      console.log(`[NFT #${tokenId}] 🔄 Refetching approval data...`);
      const approvedResult = await refetchApproved();
      const approvedForAllResult = await refetchApprovedForAll();

      console.log(`[NFT #${tokenId}] 🔄 Refetch complete:`, {
        approvedAddress: approvedResult.data,
        isApprovedForAll: approvedForAllResult.data,
        marketplaceAddress,
      });

      // Set approving state to false after refetch completes
      setIsApproving(false);
      console.log(`[NFT #${tokenId}] ✅ isApproving set to false`);
  ...
  const handleApprove = async () => {
    console.log(`[NFT #${tokenId}] 🚀 handleApprove called`, {
      marketplaceAddress,
      currentApprovalStatus: { approvedAddress, isApprovedForAll, isApproved },
    });

    try {
      setIsApproving(true);
      console.log(`[NFT #${tokenId}] ⏳ isApproving set to true`);

      await approveMarketplace();
      console.log(`[NFT #${tokenId}] 📤 Approval transaction sent`);
      ...
      console.error(`[NFT #${tokenId}] ❌ Approval failed:`, error);
  ...
  const handleList = async () => {
    console.log(`[NFT #${tokenId}] 🛒 handleList called`, {
      listPrice,
      currentState: {
        isApproved,
        isApproving,
    ...
    try {
      console.log(`[NFT #${tokenId}] 📤 Calling listItem with price:`, parseEther(listPrice));
      await listItem();
      console.log(`[NFT #${tokenId}] ✅ NFT listed successfully!`);
      notification.success("NFT listed successfully!");
      setShowListModal(false);
      setListPrice("");
      setTimeout(() => refetchListing(), 2000);
    } catch (error) {
      console.error(`[NFT #${tokenId}] ❌ Listing failed:`, error);
  ...
  // Log button state
  console.log(`[NFT #${tokenId}] 🎨 Render state:`, {
    isOwner,
    isListed,
    isApproved,
    isApproving,
